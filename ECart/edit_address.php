				...
				<?php if(isset($_REQUEST['save']))
						{
							...
							if(($fname!="")&&($lname!="")&&($email!="")&&($telephone!="")&&($add1!="")&&($add2!="")&&($state!="")&&($city!="")&&($postcode!=""))
							{
								...
								$result=mysql_query("UPDATE addresses set firstname='$fname',lastname='$lname',telephone='$telephone',address1='$add1',address2='$add2',pincode='$postcode',state='$state',city='$city'");					
								if($result>0);
								{
									$error="Address edited successfully. <br>";						
								}
								echo mysql_error();
							}
						}?>
	...
	<div class="container">
          	<div class="category-tab2">
               ...
               <div class="header-heading"> 
					<h1>Edit Address <a href="myaccount.php">Back</a></h1>
					</div>
      					<div class="row">
       ...
       <div class="name-left">
        ...
        <p><input name="firstname" type="text" class="input_email" value="<?php echo $row['firstname']?>"/></p></div>
        <div class="name-right">
        <label>Last Name <span class="star">*</span></label>
        <p><input name="lastname" type="text" class="input_email" value="<?php echo $row['lastname']?>"/></p>
        </div>
        </div>
        <label>Address 1<span class="star">*</span></label>
        <p>
          <textarea name="address1" class="shipping_data_textarea" maxlength="300" ><?php echo $row['address1']?></textarea>
        </p>
		<label>Address 2<span class="star">*</span></label>
        <p>
          <textarea name="address2" class="shipping_data_textarea" maxlength="300"><?php echo $row['address2']?></textarea>
        </p>
		<div class="name-sec">
       <div class="name-left">
          <label>City <span class="star">*</span></label>
        <p>
		<input name="city" type="text" class="input_email" value="<?php echo $row['city']?>"/></p>
        ...
        <p><input name="postcode" type="text" class="input_email" value="<?php echo $row['pincode']?>"/></p>
        </div>
       ...
       <div class="name-left">
        ...
        <p><input name="state" type="text" class="input_email" id="state" value="<?php echo $row['state']?>"/>
		</p>
		<span id="email_msg"></span>
        </div>
        <div class="name-right">
          <label>Contact Number <span class="star">*</span> </label>
        <p><input name="telephone" type="text" class="input_email" value="<?php echo $row['telephone']?>"/></p>
	<input type="hidden" value="" name="address_id">
        </div>
        </div>
		</div>
		<input name="email" type="text" class="input_email" id="email" value="<?php echo $row['email']?>" style="visibility: hidden"/>
		<?php } ?>
...
{
	if($("#"+id+" option:selected").text() == 'Others')
	{
		$("#new_city").show();
		$("#new_city").rules("add",{
		required:true,	
